...
<script>
	import { getRankSongsList } from '@/server'
	var rankSongList={
		data(){
			return{
		...
		async created() {
			let songs1 = await getRankSongsList(this.rankid,1);
		...
		methods:{
			loadBottom() {
		      ...
		      async getList(){
		      	this.page++;
		      	let songsdata = await getRankSongsList(this.rankid,this.page);
		      ...
		      savelist(item,index){
				...
				this.$router.push({path:'/rank/song/player', query:{songindex:index, songhash:item.hash, albumid:item.album_id, tab:'1'}});
			}
		}
	}
